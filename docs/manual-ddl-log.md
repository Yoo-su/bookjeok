...
> **2026-09-23 제거.** 검색이 검색 키 표현식 인덱스로 옮겨 가 쓰이지 않게 됐습니다(10절).
> 아래는 도입 당시 기록입니다.
...
## 10. 검색 키 표현식 인덱스 (적용일 미상 · 2026-09-23 기록)

### 경위

다른 세션(회사 PC)에서 띄어쓰기 무시 검색을 만들다 멈춘 작업의 흔적입니다.
**인덱스는 운영에 만들어졌지만 기록도, 그것을 쓰는 코드도 없었습니다.** 2026-09-23에
`pg_stat_user_indexes`로 보니 `idx_scan = 0`이었고, 19MB를 차지하며 쓰기 때마다
갱신되기만 하고 있었습니다. 만든 날짜는 Postgres가 남기지 않아 알 수 없습니다.
계획서와 적재 도구 README는 이것을 "`search_key` generated column"이라고 적고
있었는데, 실제로는 **컬럼이 아니라 표현식 인덱스**입니다.

같은 날 이 인덱스를 쓰는 검색 코드를 넣었습니다(`book-search-query.ts`, 계획서 8-e).

### 정의 (운영 `pg_get_indexdef` 그대로)

```sql
CREATE INDEX "IDX_books_search_key_trgm" ON public.books USING gin ((
  lower(regexp_replace(title::text,     '[^[:alnum:]]+', '', 'g')) || ' ' ||
  lower(regexp_replace(author::text,    '[^[:alnum:]]+', '', 'g')) || ' ' ||
  lower(regexp_replace(publisher::text, '[^[:alnum:]]+', '', 'g'))
) gin_trgm_ops);
```

제목·저자·출판사에서 글자·숫자가 아닌 것(공백 포함)을 지우고 소문자로 바꿔 공백
하나로 잇습니다. 그래서 "사탄 탱고"와 "사탄탱고"가 같은 값이 되고, 필드 경계의 공백
때문에 한 단어가 두 필드에 걸쳐 맞지는 않습니다. 운영 로케일(en_US.UTF-8)에서
`[:alnum:]`은 한글·자모를 남기고 ①·Ⅱ 같은 기호형 숫자는 지웁니다.

> **코드의 `SEARCH_KEY_SQL`과 이 식은 문자 하나까지 같아야 합니다.** 다르면 에러 없이
> 풀스캔이 됩니다. 식을 바꾸려면 인덱스를 새로 만들고 코드를 같은 배포에 실으세요.
> `book-search-query.spec.ts`가 코드 쪽 식을 고정합니다.

### 크기와 효과 (2026-09-23 실측)

| 항목       | 값                                                                                |
| ---------- | --------------------------------------------------------------------------------- |
| 크기       | 19MB (DB 407MB / 무료 한도 500MB)                                                 |
| 3글자 이상 | 인덱스 사용. "라슬로 사탄" 1.1ms(DB 실행), 체감 16~38ms로 기존과 같음             |
| 2글자 이하 | 인덱스를 못 탐(3-gram 한계). 기존과 같이 풀스캔이지만 쿼리 개선으로 약 2배 빨라짐 |

### 컬럼별 trgm 인덱스 3개 제거 (2026-09-23 완료)

새 검색 코드는 4절의 컬럼별 인덱스를 쓰지 않습니다. 짧은 단어는 원문 `ILIKE`를 쓰지만
2글자 패턴은 원래 인덱스를 못 탑니다. 리뷰·판매글 검색의 `book.title ILIKE`는 작은
테이블에서 조인해 들어오므로 역시 쓰지 않습니다. **지우면 약 37MB(DB의 9%)를 되찾습니다.**

**반드시 검색 키 코드가 운영에 배포된 뒤에 실행하세요.** 옛 코드는 이 인덱스로 3글자
이상 검색을 30ms에 처리합니다. 먼저 지우면 배포 전까지 모든 검색이 풀스캔이 됩니다.

```sql
-- 배포 확인 후. CONCURRENTLY는 트랜잭션 블록 밖에서 한 문장씩 실행합니다.
DROP INDEX CONCURRENTLY IF EXISTS "IDX_books_title_trgm";
DROP INDEX CONCURRENTLY IF EXISTS "IDX_books_author_trgm";
DROP INDEX CONCURRENTLY IF EXISTS "IDX_books_publisher_trgm";
```

실행 전 확인: 배포(PR #580) 뒤 세 인덱스의 `last_idx_scan`이 배포 전 시각(12:35·12:54 UTC)에
멈춰 있었고, 검색 키 인덱스만 늘고 있었습니다(14:34 UTC 기준 263회). 사용자가 Supabase SQL
에디터에서 한 문장씩 실행했습니다.

| 항목           | 전    | 후                                                  |
| -------------- | ----- | --------------------------------------------------- |
| DB 전체        | 407MB | **371MB**                                           |
| `books` 인덱스 | 59MB  | **23MB** (검색 키 19MB + PK 3.5MB)                  |
