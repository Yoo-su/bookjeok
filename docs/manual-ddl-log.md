...
| **대기**   | 컬럼별 trgm 인덱스 3개 제거 — 검색 키 코드 배포 **후** 실행                        | 10절                    |
...
실행 전에 `pg_stat_user_indexes`로 세 인덱스의 `idx_scan`이 배포 뒤 늘지 않는지
한 번 더 보세요. 실행한 뒤에는 위 적용 이력 표의 **대기**를 날짜로 바꾸고 4절 머리에
"2026-mm-dd 제거"를 적으세요.
