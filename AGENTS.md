...
- **2026-09-08에 공급처 체인에서 알라딘 어댑터를 제거했습니다.** 검색·상세 모두 자체 DB 단독이며, 외부 공급처를 런타임 경로에 두지 않는 것이 방침입니다. 신규 도서는 서버가 아니라 **운영자가 필요할 때 돌리는 적재 도구**로 확보합니다. 카카오 책 검색 API를 입구로 쓰는 [`tools/book-ingest/`](tools/book-ingest/README.md)이며(계획서 6-d 「운영자 적재 도구」), 2026-09-23에 구현해 운영에 첫 적재(1권)까지 확인했습니다. 이 도구는 `books`에 SELECT·INSERT만 하고, 표지를 R2에 먼저 올린 뒤 INSERT합니다. `resolveBook()`은 "찾거나 404"인 가드일 뿐 더는 도서를 생성하지 않습니다.
