...
├── mutations/
├── stores/
│   ├── use-tower-settings-store.ts   # 책탑 키·캐릭터·비교 작가 (기기에만 저장)
│   └── use-reading-log-view-store.ts # 마지막으로 본 보기 (달력·리스트·책탑)
├── __tests__/                        # queries · mutations · day-details-dialog · tower-height-card · tower-comparison
└── components/
    ├── calendar-view/
    ...
    │   ├── tower-book-dialog/
    │   ├── tower-share-dialog/       # 공유 이미지 (Canvas), 제목 넣을 책 고르기
...
- **같은 축척**: 무대의 책탑과 캐릭터는 같은 px/mm로 그립니다. 캐릭터는 인체 비율(무릎 28%·허리 60%·어깨 82%)이라 말풍선의 "허리까지 9cm"가 그림과 맞습니다. 작가 말풍선과 진행 눈금은 키 대비 퍼센트를 표시합니다.
...
- **공유 이미지의 책 제목**: 탑 안의 책은 한 권이 10px 안팎이라 제목을 쓸 수 없습니다. 바닥선 아래에 「쌓은 책」 목록(`lib/legend.ts`)을 범례처럼 두고, 줄마다 탑과 같은 손그림 책을 표지색으로 붙여 탑과 잇습니다. 목록 높이만큼 장면을 줄입니다. 기본은 탑 맨 위(최근) 5권이고 다이얼로그의 「제목 넣을 책」에서 최대 5권(`LEGEND_MAX`)까지 바꾸며, 나머지는 「외 N권」으로 적습니다. 모두 빼면 목록 없이 그립니다. 부제(` - `, ` : ` 뒤)와 끝 괄호는 떼고 적습니다.
  - 제목마다 화살표로 탑의 책을 가리키는 방식은 시도했다가 버렸습니다. 맨 위 몇 권이 수십 px 안에 몰려 화살표가 한곳으로 모여 꼬입니다. 무대 안 빈 곳에 목록을 두는 것도 「남은 높이」 표시와 캐릭터 사이가 좁아(스토리는 더 좁음) 들어가지 않습니다.
...
- **작가와 비교**: 키 설정 팝오버에서 내 키 또는 카뮈(약 176cm)·사르트르(약 153cm)·쿤데라(약 185cm)·울프(약 170cm)·카프카(약 182cm)를 고릅니다. 흔히 소개되는 추정값이며 설정에 출처 링크를 제공합니다. 선택 작가는 기기에 저장하고 내 키·남녀 캐릭터는 별도로 보존합니다(`use-tower-comparison`). 무대·진행률·화면/공유 문구가 같은 비교 대상을 사용합니다.
...
- **색**: 그림(캐릭터·책·눈금자)은 먹색, 측정 표시(내 키 점선·남은 높이·말풍선 둘째 줄)와 진행률·선택 상태는 서비스 포인트색 emerald-700(`#047857`)입니다. 제목 숫자 밑줄은 먹색 연필 획(stone-800)과 옅은 덧선(stone-400)이며 공유 이미지도 같습니다. 한때 파랑(`#2563EB`)이었으나 서비스에서 책탑만 쓰던 색이라 바꿨습니다.
- **움직임**: 캐릭터 선과 말풍선 테두리를 세 벌 번갈아 보여 떨리게 하고(`globals.css`의 `tower-boil`), 동작 줄이기 설정이면 멈춥니다. 말풍선은 둥근 사각형 모양 그대로 선만 연필로 긋고, 글자는 움직이지 않습니다. 보이지 않는 탭에서는 인트로를 건너뜁니다. 캐릭터 목록은 Storybook `ReadingLog/TowerFigures`에서 한눈에 봅니다.
