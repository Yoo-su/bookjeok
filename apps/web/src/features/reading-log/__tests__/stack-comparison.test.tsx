...
import { useStackComparison } from "@/features/reading-log/components/stack-view/hooks/use-stack-comparison";
import { useStackCopy } from "@/features/reading-log/components/stack-view/hooks/use-stack-copy";
import {
  STACK_AUTHOR_IDS,
  STACK_AUTHORS,
} from "@/features/reading-log/components/stack-view/lib/authors";
...
import { useStackSettingsStore } from "@/features/reading-log/stores/use-stack-settings-store";
...
beforeEach(() => {
  useStackSettingsStore.setState({
    heightCm: 168,
    character: "F",
    author: null,
  });
...
describe("독서 키재기 비교 대상", () => {
  it("작가를 번갈아 골라도 입력한 내 키와 캐릭터는 유지한다", () => {
    ...
    for (const author of [
      ...
      expect(useStackSettingsStore.getState().heightCm).toBe(168);
    }
  ...
  it("화면과 공유 문구에 선택한 작가와 추정 키를 표시한다", () => {
    useStackSettingsStore.setState({ author: "kafka" });
    const { result } = renderHook(useStackCopy, { wrapper });
    const status = stackStatus(910, 1820);
...
describe.each(["M", "F"] as const)(
  ...
  (character) => {
    it("높이에 따라 표정과 대사가 함께 바뀐다", () => {
      useStackSettingsStore.setState({ author: null, character });
      const { result } = renderHook(useStackCopy, { wrapper });
      ...
      for (const [ratio, mood] of [
        ...
        const status = stackStatus(ratio * 1700, 1700);
...
describe("작가 캐리커처", () => {
  ...
  it("화면에서는 선을 세 벌 그려 떨리게 하고, 공유 이미지는 한 벌만 그린다", () => {
    ...
    expect(classes(figure("kafka", { boil: true }))).toEqual([
      "stack-boil stack-boil-0",
      "stack-boil stack-boil-1",
      "stack-boil stack-boil-2",
    ]);
    expect(classes(figure("kafka"))).toEqual(["stack-figure"]);
  });

  it("기본 캐릭터처럼 가장 최근에 읽은 책 색을 손에 든다", () => {
    for (const id of STACK_AUTHOR_IDS) {
