...
/**
 * 키 대비 몸 부위 높이. 캐릭터도 이 비율로 그려서 쌓은 책 꼭대기가
 * 몸의 어디쯤인지 그림과 말이 맞는다.
 */
...
export function stackStatus(stackMm: number, userMm: number): StackStatus {
  const ratio = userMm > 0 ? stackMm / userMm : 0;
  const passed =
  ...
  const toNextCm = nextPart
    ? Math.max(1, Math.ceil((nextPart.ratio * userMm - stackMm) / 10))
    : 0;
  const mood: Mood =
    ratio >= 1
      ? "wow"
  ...
  return {
    ...
    toNextCm,
    overCm: ratio >= 1 ? Math.round((stackMm - userMm) / 10) : 0,
    mood,
  };
}
