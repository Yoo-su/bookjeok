...
/**
 * 공유 이미지 바닥에 적는 「쌓은 책」 목록. 차트의 범례처럼 제목마다 쌓은 책과 같은
 * 손그림 책을 표지색으로 붙여, 화살표 없이도 쌓은 책과 이어 보이게 한다.
 */
 ...
 * books는 위에서부터 쌓인 순서다. 좌표는 목록 왼쪽 위가 원점이다.
 */
export function buildLegend(o: {
  ...
  for (const b of o.books) {
    ...
    // 쌓은 책과 같은 그림: 색면을 윤곽에서 살짝 어긋나게 찍고 흔든 윤곽을 두른다
