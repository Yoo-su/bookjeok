...
/**
 * 독서 키재기 공유 이미지. 화면과 같은 장면을 1080px 폭 캔버스에 다시 그린다.
 * 서버 렌더링 없이 브라우저에서 바로 만든다.
 */
export async function renderStackShareImage(o: {
  format: ShareFormat;
  ...
  // 높이 숫자 아래 펜 밑줄. 화면(reading-stack)과 같은 100×12 도형을 늘려 그린다
  const ul = hw + uw + 20;
  ...
  // 쌓은 책 목록. 쌓은 책은 한 권이 너무 얇아 제목을 못 쓰니 바닥 아래 띠에 모아 적고,
  // 그 높이만큼 장면을 줄인다. 위에서부터 쌓인 순서로 적는다
