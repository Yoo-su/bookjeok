...
import { TOWER_AUTHOR_IDS, TOWER_AUTHORS } from "../lib/authors";
...
export function TowerHeightChip({ className }: { className?: string }) {
  ...
  const { author, setAuthor, heightCm, isDefaultHeight } = useTowerComparison();
  return (
    <Popover>
            ...
            {TOWER_AUTHOR_IDS.map((id) => (
              ...
              >
                ...
                <span className="shrink-0 text-xs tabular-nums">
                  {t("approx_height", { cm: TOWER_AUTHORS[id].heightCm })}
                </span>
              </button>
            ))}
          </div>
          <p className="text-xs leading-relaxed text-stone-500">
            ...
            {author && (
              <>
                {" "}
                <a
                  href={TOWER_AUTHORS[author].source}
                  target="_blank"
                  rel="noreferrer"
