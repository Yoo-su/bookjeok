...
- 인기 리뷰 카드도 상세로 가는 실제 `Link`입니다. 클릭 시 공통 이동 진행 표시기가 작동하고 키보드·새 탭 이동이 가능하며, 목록 카드이므로 `prefetch={false}`를 유지합니다.
...
- **카운트 교정**: 성공하면 서버 응답의 `reactionCounts`로 덮어씁니다. 상세는 ISR(24시간) 캐시라 페이지 진입 직후엔 카운트가 낡아 있을 수 있고, 다른 사용자의 반응이 겹치기도 합니다. 응답 전체가 아니라 카운트만 덮는 이유는 비공개 리뷰의 토글 응답이 본문을 가린 채 오기 때문입니다.
- **실패**: 낙관적 반영을 되돌리고 `review.toast.reaction_error` 토스트를 띄웁니다.
- **내 리액션**: 상세와 별개로 `GET /reviews/:id/reaction`을 인증해 조회합니다(캐시 키 `[...reviewKeys.detail(id), "reaction"]`). 키에 사용자 ID가 없지만, 로그아웃·로그인이 모두 하드 내비게이션이라 이전 사용자의 캐시가 남지 않습니다. SPA 이동으로 계정을 바꾸는 경로를 만들면 키에 사용자 ID를 넣어야 합니다.

## 3. SEO

`review-json-ld`가 리뷰 상세에 구조화 데이터를 삽입하고, 리뷰 피드는 `/rss.xml`에도 포함됩니다.

태그는 두 곳에 함께 실립니다(2026-09-21).

- **JSON-LD `keywords`** — 태그를 쉼표로 이어 붙입니다. 본문 발췌보다 압축된 주제 신호입니다.
  태그가 없으면 속성 자체를 만들지 않습니다.
- **RSS `<category>`** — 항목당 여러 개를 냅니다. 리뷰는 `도서리뷰` + 태그 전부, 판매글은
  `중고도서` 하나입니다. 네이버가 RSS를 신규 웹문서 수집 소스로 쓰는데 고정값 하나만
  실으면 글마다 주제 구분이 없습니다.

**둘 다 순위를 직접 올리는 장치가 아닙니다.** 태그가 검색 유입을 만들려면 태그마다 고유
URL이 있어야 하는데, 2026-09-21 실측에서 2회 이상 쓰인 태그가 18개뿐이라 랜딩 페이지는
보류했습니다. 3회 이상 태그가 20~30개쯤 쌓이면 그때 다시 봅니다(현재 7개).
