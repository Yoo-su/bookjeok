...
import { ReadingTowerIntro } from "@/features/announcement/components/reading-tower-intro";
...
let towerItems: typeof SAMPLE_BOOKS = [];
vi.mock("@bookjeok/react-query", () => ({
  useReadingTowerQuery: () => ({ data: { year: 2026, items: towerItems } }),
}));

// 무대는 ResizeObserver와 SVG 측정이 필요해 누가 서는지만 남긴다
vi.mock("@/features/reading-log/components/tower-view/tower-stage", () => ({
  TowerStage: (p: { books: unknown[]; person?: { character: string } }) => (
    <div
      data-testid="stage"
...
function renderIntro() {
  ...
  render(
    <NextIntlClientProvider locale="ko" messages={messages}>
      <ReadingTowerIntro open onOpenChange={onOpenChange} />
...
describe("ReadingTowerIntro", () => {
  ...
  it("비로그인은 예시 책탑으로 소개하고, 가입하면 책탑으로 돌아오게 한다", async () => {
    const onOpenChange = renderIntro();
    expect(
      await screen.findByText("올해 읽은 책, 쌓아 보면 얼마나 될까요?"),
    ).toBeInTheDocument();
    ...
    await toLast();
    fireEvent.click(screen.getByRole("button", { name: "가입하고 책탑 쌓기" }));
    expect(saveReturnUrl).toHaveBeenCalledWith("/my-page/reading-log");
    expect(push).toHaveBeenCalledWith("/login");
    expect(useReadingLogViewStore.getState().viewMode).toBe("tower");
    expect(onOpenChange).toHaveBeenCalledWith(false);
  });

  it("로그인했고 올해 기록이 있으면 내 책탑 높이로 소개하고 내 책탑으로 보낸다", async () => {
    useAuthStore.setState({ accessToken: "token" });
    towerItems = SAMPLE_BOOKS.slice(0, 3);
    const mm = towerItems.reduce((a, b) => a + b.depth, 0);
    renderIntro();
    expect(
    ...
    await toLast();
    fireEvent.click(screen.getByRole("button", { name: "내 책탑 보러 가기" }));
