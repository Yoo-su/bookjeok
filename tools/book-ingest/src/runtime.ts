...
import { createKakaoSearch } from "./kakao";
...
export function createScanDeps(db: BookDb): ScanDeps {
  return {
    search: createKakaoSearch(requireEnv("KAKAO_REST_API_KEY")),
    findExisting: (isbns) => db.findExisting(isbns),
  };
}

/** R2 자격증명은 적재할 때만 필요합니다. 조회만 할 때는 없어도 됩니다. */
export function createIngestDeps(db: BookDb): IngestDeps {
